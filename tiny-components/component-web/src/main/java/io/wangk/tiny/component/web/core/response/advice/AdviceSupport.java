...
import io.wangk.tiny.component.web.core.response.api.ExcludeResponse;
...
public class AdviceSupport {
    ...
    /**
     * 判断方法是否命中@ExcludeResponse注解
     *
     * @param method
     * @return
     */
    public boolean matchExcludeResponse(Method method) {

        //有ExcludeFromGracefulResponse注解修饰的，也跳过
        if (method.isAnnotationPresent(ExcludeResponse.class)) {
            logger.debug("Response:方法被@ExcludeResponse注解修饰:methodName={}", method.getName());
            return true;
        }

        //有ExcludeFromGracefulResponse注解修饰的类，也跳过
        if (method.getDeclaringClass().isAnnotationPresent(ExcludeResponse.class)) {
            logger.debug("Response:类被@ExcludeResponse注解修饰:methodName={}", method.getName());
            return true;
        }

        //有ExcludeFromGracefulResponse注解修饰的返回类型，也跳过
        if (method.getReturnType().isAnnotationPresent(ExcludeResponse.class)) {
            logger.debug("Response:返回类型被@ExcludeResponse注解修饰:methodName={}", method.getName());
            return true;
        }
        //未命中，返回false
        return false;
