<template>
  <div
    class="flex justify-between tags-container"
    ref="containerDom"
    v-if="appStore.tag"
  >
    <div
      class="menu-tags-wrapper"
      ref="scrollbarDom"
      :class="{ 'tag-pn': tagShowPrevNext }"
    >
      <div class="tags" ref="tags">
        <a
          v-for="tag in formatTags(tagStore.tags)"
          :key="tag.path"
      ...
      <span class="ma-tag-prev" v-if="tagShowPrevNext">
        <IconLeft
          :size="20"
          class="tag-scroll-icon"
          @click="handleScroll(-500)"
        />
      </span>
    ...
    <a-trigger
      class="ma-tags-more-dropdown"
      :popup-translate="[-65, -6]"
      :show-arrow="true"
      trigger="hover"
    >
...
<script setup>
import { ref, watch, onMounted, nextTick, KeepAlive } from "vue";
...
onMounted(() => {
  if (tags.value) {
    ...
    tags.value.addEventListener("mousewheel", scrollHandler, {
      passive: true,
    }) ||
      tags.value.addEventListener("DOMMouseScroll", scrollHandler, {
        passive: true,
      });
    nextTick(() => {
      tagShowPrevNext.value = tags.value.scrollWidth > tags.value.offsetWidth;
