...
/**
 * 根据新列顺序对原始列进行排序
 * @param {Array} originColumns 原始列数组
 * @param {Array} newColumns 新列数组，用于指定排序后的列顺序
 */
export function sortColumns(originColumns, newColumns) {
  ...
  if (newColumns.length === 0) {
    ...
    // 查找原始列中的“选择”列，并准备重新填充原始列数组
    const selectionItem = originColumns.find((it) => it.type === "selection");
    originColumns.length = 0; // 清空原始列数组以重新填充
    if (selectionItem) {
      // 如果找到“选择”列，则首先将其添加到原始列数组中
      originColumns.push(selectionItem);
    }
    // 将新列数组中的列追加到原始列数组中，以完成排序
