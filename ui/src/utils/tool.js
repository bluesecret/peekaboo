...
tool.viewImage = function (path, defaultStorage = "LOCAL") {
  let mode = tool.local.get("site_storage_mode")
    ? tool.local.get("site_storage_mode").toUpperCase()
    : defaultStorage;
...
tool.download = (res, downName = "") => {
  ...
  if (res.headers && res.data) {
    blob = new Blob([res.data], {
      type: res.headers["content-type"].replace(";charset=utf-8", ""),
    });
    if (!downName) {
      const contentDisposition = decodeURI(res.headers["content-disposition"]);
      const result = contentDisposition.match(/filename=(.+)/gi);
      fileName = result[0].replace("filename=", "");
    }
  }
