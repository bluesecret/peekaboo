    ...
    <div class="_crud-header flex flex-col mb-2" ref="crudHeaderRef">
      ...
      >
        ...
        <a-tab-pane
          :key="item.value"
          :title="item.label"
          v-for="item in options.tabs.data"
        >
          <template #title
            ><slot :name="'tabTitle-' + item.label">{{
              item.label
            }}</slot></template
          >
        </a-tab-pane>
          ...
          >
            ...
            <template #columns>
              ...
              >
                ...
                <template
                  v-for="slot in getTitleSlot(columns)"
                  #[slot]="{ column }"
                >
...
const refresh = async () => {
  if (props.data) {
    loading.value = true;
    const data = isArray(props.data)
      ? props.data
      : config.parseResponseData(await props.data(requestParams.value));
    if (data.rows) {
...
const searchSubmitHandler = async (formData) => {
  if (
    options.value.requestParamsLabel &&
    requestParams.value[options.value.requestParamsLabel]
  ) {
...
const settingFixedPage = () => {
  ...
  const tableHeight =
    workAreaHeight - headerHeight.value - (openPagination.value ? 152 : 108);
...
const getColumnService = (strictMode = true) => {
  return new ColumnService(
    { columns: columns.value, cascaders: cascaders.value, dicts: dicts.value },
    strictMode
  );
};
